...
motif_list[12,2] <- '[^RK] in sequence'

rows <- grep('[RK]', gcn4$sequence)
hold <- gcn4[-rows,]
...
#saveRDS(motif_list,'motif_list_with_live%_RK.rds')
#write.csv(motif_list,'motif_list_with_live%_RK.csv')
...
ggplot(coef, aes(xaxis, coef,fill = color))+
  ...
  ggtitle('Gcn4', subtitle = 'Ridge ROC AUC: 0.733')

#write.csv(coef, 'figure1_ML_gcn4_coeff_plus_basic_RK.csv')
...
#write.csv(coef, 'figure1_ML_gcn4_coeff_without_basic_RK.csv')
